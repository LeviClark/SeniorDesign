...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerVer1NetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerVer1NetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerVer1NetworkObject.Length > 0 && PlayerVer1NetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerVer1NetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerVer1Behavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerVer1 instead, its shorter and easier to type out ;)")]
		public PlayerVer1Behavior InstantiatePlayerVer1NetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerVer1NetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerVer1Behavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerVer1Behavior>().networkObject = (PlayerVer1NetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerVer1
		/// </summary>
		/// <returns>
		/// A local instance of PlayerVer1Behavior
		/// </returns>
		/// <param name="index">The index of the PlayerVer1 prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerVer1Behavior InstantiatePlayerVer1(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerVer1NetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerVer1Behavior>();
